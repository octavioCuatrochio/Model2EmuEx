...
| 5 | network board 0x1a10000–0x1a1ffff, page 0x1a1 (0x4c7ae0–0x4c7ba0) | Only the 8-bit read checks whether networking is active (flag set when `m2network.ini` networking starts, 0x4cf960). The 16- and 32-bit reads always return shared RAM, which is at `M2ZSHARED + 0x8000`. With networking off, an 8-bit read of 0x1a14000 returns 0xff: bit 0 set means "no board". The 16-bit read returns the upper half of the host pointer in the high word, which a 16-bit bus drops. Registers: 0x1a14000 (bit 0: board on, written by the game) and 0x1a14002 (bit 0: master, written by the game; bit 7: heartbeat, toggled every slice and on every packet). Shared RAM 0x1a10000–0x1a13fff. Until the `coop` branch the port mapped these handlers on page 0x1a0 by mistake; single-player output is identical either way (checked with `M2_GEOHASH`), because single mode never probes the board. | confirmed |
...
| 8 | texture filtering, mipmaps, microtextures, texture LOD | Bilinear and optional trilinear filtering in Direct3D. | `--texture-filter` (F6): nearest texel (default, as MAME), bilinear or trilinear, done in the shader because the atlas holds texel indices that only become colours through the luma and colour tables: each neighbouring texel is looked up to its colour and those are blended. Texels outside the texture repeat, or clamp on mirrored axes, so neighbouring textures in the atlas don't bleed in; translucent texels (15) count as transparent and the pixel is dropped under half coverage. Trilinear can't use mipmaps (the colours depend on each polygon's tables): where a pixel covers more than one texel it averages four bilinear samples over the pixel's footprint (screen-space derivatives), which removes the shimmer mipmaps would. Needs highp in the fragment shader: on the Mali-400 (mediump only) nearest is the right choice. No microtextures. |
...
| 18 | picture shape (`m2emu --aspect`, `--widescreen fill`, `--split`) | The window's shape, with the Lua widescreen rules. | For a rectangle narrower than the picture (split screen side by side): keep (bands), stretch (squeezed), or crop: the frame is made narrower than 496 (the centre of the 3D, shifted by a negative `wide_extra`, and of the sky layer, B low priority), while layer A and B's high-priority pixels (the HUD) are drawn squeezed over the whole narrower frame. Crop only applies while the widescreen rule says the game shows full-screen 3D; other screens are squeezed. `fill` gives the frame the rectangle's own ratio (at most 4:1). |
...
## 6. Network: linked boards (`m2net.c`, `m2emu --coop`)

The original links emulator instances in a ring over UDP (`m2network.ini`: RxPort, NextIP, NextPort, FrameSync; thread 0x4cf740, receive 0x4cf3d0, send at the end of each frame in 0x4cc7a0). The port links boards in one process instead, run in lockstep: all boards finish frame N, `m2net_frame` does what the network thread would have done meanwhile, then all start frame N + 1.

| # | Where | Original | Port |
|---|-------|----------|------|
| 1 | search (state 0) | Once the game turns the board on, the thread polls the socket without waiting and gives up after 60 empty polls: heartbeat bit set, net RAM +0 = 0, +2 = 0xffff, +0x12 = 0xe00. Then state 1, master = register 0x1a14002 bit 0, slot 0x1c0 (mode 0x574ddc, whose initial value is 1), +0x14 = 0x1c0, block cleared. | The same, at the first frame boundary after the game turns the board on. |
| 2 | link-up (packet types 0 and 1) | The master sends "M2EM" type 0 every 100 ms; each slave adds its address and passes it on; back at the master it becomes type 1 with byte 6 = 2, and each slave takes byte 6 as its position and increments it. Numbers: master 1 (+2), the others count - position + 2; +3 = the count; +0 = 1; heartbeat bit set; state 3. A node not listening yet drops the packet. | The same numbers, assigned at the first frame boundary where every node is listening and exactly one is the master. |
| 3 | running (type 2) | The master sends its 0xe00-byte block (net RAM +0x2000) at the end of each frame; each slave, on receiving its predecessor's block, sends its own on, then stores the received block at +0x2000 + slot (0x1c0) and toggles the heartbeat; the master stores the last slave's block. With FrameSync the frame end waits (up to 2 s) for that. | Every node's block is taken at the frame boundary and stored in the next node's slot, heartbeats toggled. Deterministic, as with FrameSync. |
| 4 | Daytona settings | Link mode (backup RAM settings +0x0b: 0 single, 1 master, 2 slave, shown as LINK ID) and CAR NUMBER (+0x0c, 0–7 shown as 1–8) are set in test mode. | `--coop` writes them into both settings copies (CRC-16 recomputed) at every start: board 1 master with car 1, board 2 slave with car 2. Each board has its own save file (`<game>-coop1.DAT`, `-coop2.DAT`). |
| 5 | UDP | Works between PCs on a LAN. | Not implemented yet; `m2net.c` only links boards in one process. |
| 6 | sound | One sound board per instance: each cabinet has its own speakers. | `--coop` plays player 1's sound board only; player 2's board runs without one (its sound commands are ignored, which the game doesn't notice). `m2snd.c` can run several sound boards (Musashi is one global 68000, so each keeps its own saved CPU context), but mixing two copies of the same music, slightly out of step, sounded wrong. |
